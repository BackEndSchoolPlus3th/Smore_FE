...
const AlarmPage: React.FC<AlarmPageProps> = ({ isOpen, onClose, events }) => {
  ...
  const renderNotification = (alarm: Alarm) => {
    return (
      <div className="p-3 rounded-lg w-58 shadow-md relative hover:scale-105 transition-transform">
        ...
        <p className="text-[13px]">{alarm.message}</p>
        ...
        {alarm.eventName === "application__reached" && (
          <>
            ...
            <div className="grid grid-cols-3 gap-2 mt-2">
              ...
              />
              <button
                className="bg-purple-600 text-white text-[10px] px-2 py-1 rounded-md hover:scale-105 transition"
                onClick={() => apply(alarm)}
              >
                수락
              </button>
  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center" onClick={onClose}>
      <div
        className="bg-white p-5 w-72 rounded-2xl shadow-lg grid grid-rows-[auto_1fr_auto] gap-4"
