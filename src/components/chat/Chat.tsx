...
const Chat: React.FC<ChatProps> = ({ roomId, chatType }) => {
  ...
  useEffect(() => {
    const handleStorageChange = () => {
      ...
      if (tokenFromStorage !== jwt) {
        ...
        console.log("새 토큰:", tokenFromStorage.substring(0, 20) + "...");
        ...
        console.log("사용자 ID 업데이트 [storage event]:", currentUserId, "->", newUserId);
  ...
  useEffect(() => {
    ...
    console.log(`채팅 히스토리 로드 중 - 방: ${roomId}, 유형: ${chatType}, 사용자: ${currentUserId}`);

    apiClient
      .get(`/api/v1/chatrooms/${chatType}/${roomId}/messages`)
      .then((response) => {
        console.log("채팅 히스토리 로드 성공:", response.data.length, "개 메시지");
  ...
  const refreshAccessTokenAndReconnect = async () => {
    ...
    try {
      const response = await apiClient.post("/api/member/refresh", { refreshToken });
      const newJwt = response.data.accessToken;

      console.log("토큰 갱신 성공 - 이전:", jwt.substring(0, 20) + "...");
      console.log("토큰 갱신 성공 - 새 토큰:", newJwt.substring(0, 20) + "...");
      ...
      console.log("사용자 ID 업데이트 [토큰 갱신]:", currentUserId, "->", newUserId);
      setCurrentUserId(newUserId);
      ...
      console.error("❌ Refresh Token 갱신 실패! 다시 로그인하세요.", error);
  ...
  const connectWebSocket = () => {
    ...
    const socket = new SockJS(`http://localhost:8090/ws?token=${jwt}`, undefined, {
            ...
            setMessages((prevMessages) => {
              ...
              if (
                ...
                (newMessages[newMessages.length - 1].type === "message" &&
                  (newMessages[newMessages.length - 1] as Message).date !== dateStr)
              ) {
      ...
      <div className="flex flex-col flex-1 bg-purple-200 p-4">
        <div className="flex-1 overflow-y-auto p-4 min-h-[500px]">
          {messages.map((msg, index) =>
            msg.type === "divider" ? (
              <div className="text-center text-gray-500 my-2" key={index}>
                📅 {msg.text}
              </div>
            ) : (
              <div 
