...
const Sidebar: React.FC<SidebarProps> = ({ selectedRoom, onRoomSelect }) => {
  ...
  const { study_id, chat_type: chatType } = useParams<{ study_id?: string; chat_type?: string }>();
  ...
  const handleChatRoomClick = async (room: ChatRoom, type: 'dm' | 'group') => {
    if (type === 'group') {
      try {
      ...
      <div className="p-4 h-full max-h-full">
        ...
        <div className="mt-4 max-h-170 overflow-y-auto">
          ...
          {expandedCategory === 'dm' && (
            <ul className="mt-2 ml-4">
              {dmRooms.map((room) => {
                const isSelected = selectedRoom?.roomId === room.roomId && chatType === 'dm';
                return (
                  <li
                    key={room.roomId}
                    className={`
                    ...
                    `}
                    onClick={() => handleChatRoomClick(room, 'dm')}
                  >
                    {room.roomName}
                  </li>
                );
        ...
        <div className="mt-4 max-h-170 overflow-y-auto">
          ...
          {expandedCategory === 'group' && (
            <ul className="mt-2 ml-4">
              {groupRooms.map((room) => {
                const isSelected = selectedRoom?.roomId === room.roomId && chatType === 'group';
                return (
                  <li
                    key={room.roomId}
                    className={`
                      ...
                      ${isSelected ? 'bg-gray-200 font-bold text-black' : 'text-gray-500'}
                    `}
                    onClick={() => handleChatRoomClick(room, 'group')}
                  >
                    {room.roomName}
                  </li>
                );
